...
const htmlModalContent = ({ id, title, description, url }) => {
  ...
  return `
    ...
    ${url
      ? `
            <img width='100%' height="150px" src=${url} alt='card image cap'  class='img-fluid mb-3 rounded-lg ' />
          `
      : `
...
const searchTask = (e) => {
  ...
  resultData.map((cardData) => {
    taskContents.insertAdjacentHTML("afterbegin", htmlTaskContent(cardData));
  });
};
